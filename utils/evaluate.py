...
def evaluate(init_score, test_score, test_label=None, q=1e-2, threshold=0):
    res = {
...
def eval_method(gt,pred,loss):
    accuracy = accuracy_score(gt, pred)
    precision, recall, f_score, support = precision_recall_fscore_support(gt, pred, average='binary')
    auc_roc = roc_auc_score(gt, loss)
    auc_pr = average_precision_score(gt, loss)

    return accuracy,precision,recall,f_score,auc_roc,auc_pr

def compute_vus(labels, score, version='opt', thre=250, **args):
    ...
    return avg_auc_3d, avg_ap_3d # VUS_ROC, VUS_PR
