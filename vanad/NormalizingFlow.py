...
class NormalizingFlow(nn.Module):
    def __init__(self, 
                 n_blocks, n_sensor, 
                 input_size, hidden_size, n_hidden, 
                 cond_label_size=None, 
                 activation='relu', 
                 input_order='sequential', 
                 batch_norm=False, mode = 'rand'):
        super().__init__()
        # base distribution for calculation of log prob under the model
        if mode == 'zero':
            self.register_buffer('base_dist_mean', torch.zeros(n_sensor))
            self.register_buffer('base_dist_var', torch.ones(n_sensor))
        elif mode == 'rand':
            self.register_buffer('base_dist_mean', torch.randn(n_sensor))
            self.register_buffer('base_dist_var', torch.ones(n_sensor))
    ...
    def base_dist(self, z, window_size):
        # C = z.shape[-1]
        # C = 51
        N =  z.shape[0]//window_size
        logp = - 0.5*(z-self.base_dist_mean.unsqueeze(0).repeat_interleave(window_size, 0).repeat(N,1))**2
    ...
    def log_prob(self, x, window_size, y=None):

        # print('shape', self.base_dist_mean.shape)
        u, sum_log_abs_det_jacobians = self.forward(x, y)
        C = u.shape[1]
        return torch.sum(self.base_dist(u, window_size) + sum_log_abs_det_jacobians, dim=1)+ C * _GCONST_ #,u
