...
class TimeLogger:
    def __init__(self,config,data_loader):
        ...
        self.dl = len(data_loader)
        self.batch_size = self.config.batch_size
        self.total_epochs = self.config.epochs
        
    def log(self,epoch,step):
        print(f"[Epoch]: {epoch+1}/{self.total_epochs} [Iter]: {step+1}/{self.dl}")
                    
        t = time.time() - self.start_time
        total_iters_done = epoch * self.dl + (step + 1)
        remaining_iters = (self.total_epochs * self.dl) - total_iters_done
        
        speed = t / total_iters_done  
        total_time = self.dl * self.total_epochs * speed
        remain_time = remaining_iters * speed
        eta_time = time.strftime("%H:%M:%S",time.localtime(time.time() + remain_time))
        
        # print(f"[Time]: {speed:.2f}s/iter")
        # print(f"[Elapsed]: {format_time(t)}")
        print(f"[Remaining]: {format_time(remain_time)}")
        # print(f"[Total Estimated]: {format_time(total_time)}")
